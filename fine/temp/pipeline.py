...
from fine.temp.gmm import fit_mixture
from fine.temp.finesvd_classifier import get_singular_vector, get_score
from fine.utils.plots import print_noise_metrics, correction_metrics, plot_score_distributions_by_class
# ----------------------------- seeds -----------------------------
...
def run_client_pipeline(
    ...
    train_loader = build_loader(ds_client, batch_size=cfg.batch_size, shuffle=True, num_workers=4, seed=cfg.seed)
    train_loader_plain = build_loader(ds_client_plain, batch_size=cfg.batch_size, shuffle=False, num_workers=4, seed=cfg.seed)
    ...
    plot_score_distributions_by_class(
        scores=scores,
        labels=y_noisy_local,          # per-sample class ids aligned with scores
        class_names=None,              # or pass list like ["cat","dog",...]
        bins=50,
        log_y=False,
        title_prefix=f"Client {cid} scores (by noisy label)",
        save_prefix=cfg.noise_type+"_"+str(cfg.level_n),              # e.g. f"client_{cid}_scores"
    )
    ...
    # ================= STAGE 1.5 =================
    print("\n" + "=" * 90)
    print(f"[STAGE 1.5] Train ONLY on predicted-clean subset | client={cid}")
    print("=" * 90)
    set_all_seeds(cfg.seed)

    # Build a dataset indexed by GLOBAL indices so pred_clean_global_idx works
    clean_only_dataset = NoisyLabelDataset(noisy_dataset.base, y_noisy=y_noisy, y_clean=y_clean,)
    # Subset to only predicted-clean GLOBAL indices
    pred_clean_global_idx = np.asarray(pred_clean_global_idx, dtype=np.int64)
    if len(pred_clean_global_idx) == 0:
        print("[STAGE 1.5] No predicted-clean samples. Skipping clean-only training.")
        test_cleanonly = None
    else:
        ds_clean_only = Subset(clean_only_dataset, pred_clean_global_idx.tolist())
        train_loader_clean_only = build_loader(
            ds_clean_only,
        ...
        net_cleanonly = build_backbone_model(cfg.model_name, cfg.pretrained, num_classes).to(device)
        net_cleanonly.load_state_dict(base_state, strict=True)

        # Train only on predicted-clean subset
        net_cleanonly, _ = train_model(net_cleanonly, train_loader_clean_only, None, device, cfg_train)
        # Evaluate
        test_cleanonly = evaluate(net_cleanonly, test_loader, device,num_classes=num_classes, label_mode="clean")
        print("[TEST]", summarize_test("clean-only", test_cleanonly))
    ...
    train_loader_corr = build_loader(ds_client_corr, batch_size=cfg.batch_size, shuffle=True, num_workers=4,seed=cfg.seed)
    ...
    train_loader_oracle = build_loader(ds_client, batch_size=cfg.batch_size, shuffle=True, num_workers=4, seed=cfg.seed)
    ...
    print(summarize_test("clean", test_cleanonly))
